...
# プーリング第一層の結果のimage_summaryを作成する
# テスト用に学習回数を少なめに設定(10)
    ...
    for i in range(10):
      ...
      if i%10 == 0:
        ...
        for i in xrange(49):
          ...
          for j in xrange(36):
            h_pool1_imgs_part[j] = tf.reshape(h_pool1_imgs_part[j], [14,14,1])
            h_pool1_imgs_part[j] = tf.image.resize_image_with_crop_or_pad(h_pool1_imgs_part[j], 16, 16)
            h_pool1_imgs_part[j] = tf.reshape(h_pool1_imgs_part[j], [1,16,16,1])
          ...
          h_pool1_part_img = tf.reshape(h_pool1_part_img, [96,96,1])
          h_pool1_part_img = tf.image.resize_image_with_crop_or_pad(h_pool1_part_img, 100, 100)
          h_pool1_part_img = tf.reshape(h_pool1_part_img, [1,100,100,1])
        ...
        h_pool1_sum = tf.image_summary("h_pool1_Visualize"+str(i), h_pool1_img, max_images=10)
        h_pool1_sum_str = sess.run(h_pool1_sum, feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})
        train_writer.add_summary(h_pool1_sum_str, i)
