...
# h_conv1_visualize.py
# 畳み込み第一層の畳み込み結果のimage_summaryを作成する
# テスト用に学習回数を少なめに設定(10)
  ...
  with tf.Session() as sess:
    ...
    for i in range(10):
      ...
      if i%10 == 0:
        ...
        (?, 28, 28, 32)
        '''
        zero_pad = tf.zeros([1,28,28,1])
        h_conv1_imgs = tf.slice(h_conv1, [0,0,0,0], [49,-1,-1,-1])
        h_conv1_imgs = tf.split(0, 49, h_conv1_imgs) # (1,28,28,32) * 49
        for i in xrange(49):
          h_conv1_imgs[i] = tf.concat(3, [h_conv1_imgs[i],zero_pad,zero_pad,zero_pad,zero_pad]) # (1,28,28,36)
          h_conv1_imgs_part = tf.split(3, 36, h_conv1_imgs[i]) # (1,28,28,1) * 36
          for j in xrange(36):
            h_conv1_imgs_part[j] = tf.reshape(h_conv1_imgs_part[j], [28,28,1])
            h_conv1_imgs_part[j] = tf.image.resize_image_with_crop_or_pad(h_conv1_imgs_part[j], 30, 30)
            h_conv1_imgs_part[j] = tf.reshape(h_conv1_imgs_part[j], [1,30,30,1])
          ...
          h_conv1_part_img = tf.reshape(h_conv1_part_img, [180,180,1])
          h_conv1_part_img = tf.image.resize_image_with_crop_or_pad(h_conv1_part_img, 200, 200)
          h_conv1_part_img = tf.reshape(h_conv1_part_img, [1,200,200,1])
        ...
        h_conv1_sum = tf.image_summary("h_conv1_Visualize"+str(i), h_conv1_img, max_images=10)
        h_conv1_sum_str = sess.run(h_conv1_sum, feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})
        train_writer.add_summary(h_conv1_sum_str, i)
