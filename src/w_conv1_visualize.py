...
# w_conv1_visualize.py
# 畳み込み第一層のフィルターのimage_summaryを作成する
# テスト用に学習回数を少なめに設定(10)
  ...
  with tf.Session() as sess:
    ...
    for i in range(10):
      ...
      if i%10 == 0:
        ...
        W_conv1_imgs = tf.concat(3, [W_conv1, zero_pad, zero_pad, zero_pad, zero_pad])
        W_conv1_imgs = tf.split(3, 36, W_conv1_imgs)

        for i in xrange(36):
          W_conv1_imgs[i] = tf.reshape(W_conv1_imgs[i], [5, 5, 1])
          W_conv1_imgs[i] = tf.image.resize_image_with_crop_or_pad(W_conv1_imgs[i], 7, 7)
          W_conv1_imgs[i] = tf.reshape(W_conv1_imgs[i], [7, 7, 1, 1])

        W_conv1_img1 = tf.concat(0, W_conv1_imgs[0:6])
        W_conv1_img2 = tf.concat(0, W_conv1_imgs[6:12])
        W_conv1_img3 = tf.concat(0, W_conv1_imgs[12:18])
        W_conv1_img4 = tf.concat(0, W_conv1_imgs[18:24])
        W_conv1_img5 = tf.concat(0, W_conv1_imgs[24:30])
        W_conv1_img6 = tf.concat(0, W_conv1_imgs[30:36])

        W_conv1_img = tf.concat(1, [W_conv1_img1, W_conv1_img2, W_conv1_img3, W_conv1_img4, W_conv1_img5, W_conv1_img6])
        W_conv1_img = tf.reshape(W_conv1_img, [1, 42, 42, 1])

        W_conv1_sum = tf.image_summary("W_conv1_Visualize"+str(i), W_conv1_img, max_images=10)
        W_conv1_sum_str = sess.run(W_conv1_sum, feed_dict={x: batch[0], y_: batch[1], keep_prob: 1.0})
        train_writer.add_summary(W_conv1_sum_str, i)
